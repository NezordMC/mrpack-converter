...
ctx.onmessage = async (event: MessageEvent<WorkerMessage>) => {
  ...
  try {
    if (type === "READ_MANIFEST") {
      ...
      isPaused = false;
      const data = event.data as { file: File; manifest: ModrinthManifest; options: ConversionOptions };
...
async function convert(file: File, manifest: ModrinthManifest, options: ConversionOptions) {
  ...
  if (options.injectedFiles && options.injectedFiles.length > 0) {
    postProgress(`Injecting ${options.injectedFiles.length} additional files...`, 10, 0);

    for (const file of options.injectedFiles) {
      const arrayBuffer = await file.arrayBuffer();

      let targetPath = file.name;
      if (file.name.endsWith(".jar")) {
        targetPath = `mods/${file.name}`;
      }

      newZip.file(targetPath, arrayBuffer);
    }
  }
